...
<style>
    ...
    h2 { color: #cbd5e1 !important; font-weight: 600 !important; letter-spacing: -0.01em; }
    ...
    /* Badge de resultado positivo / negativo */
    .badge-pos { color: #10b981; font-weight: 600; font-family: 'JetBrains Mono', monospace; }
    .badge-neg { color: #ef4444; font-weight: 600; font-family: 'JetBrains Mono', monospace; }
    .badge-neu { color: #94a3b8; font-weight: 600; font-family: 'JetBrains Mono', monospace; }

    /* Ticker chip na sidebar */
    .ticker-chip {
    ...
    if df.empty:
        ...
        with col_left:
            ...
            if not df_carteira.empty:
                ...
                pie = alt.Chart(df_chart).mark_arc(innerRadius=60, outerRadius=130).encode(
...
# ─────────────────────────────────────────────
    ...
    if not df_carteira.empty:
        ...
        c3.metric("Maior Posição", df_carteira.loc[df_carteira['Valor Investido'].idxmax(), 'Ativo'])
        ...
        st.dataframe(
            df_view.style
                ...
                .background_gradient(subset=['Valor Investido'], cmap='Blues'),
    ...
    if not df_mensal.empty:
        ...
        st.dataframe(
            df_mensal_display.style
                ...
                .applymap(color_result, subset=['Resultado (R$)']),
            use_container_width=True,
            hide_index=True
        )
    ...
    if df.empty:
        ...
        with col_a:
            ...
            if not df_carteira_bdr.empty:
                ...
                if not df_pos.empty:
                    row_p = df_pos.iloc[0]
                    st.metric("Quantidade em Carteira", int(row_p['Quantidade']))
                    st.metric("Preço Médio", fmt_brl(row_p['Preço Médio']))
                    st.metric("Custo Total", fmt_brl(row_p['Valor Investido']))
                else:
...
# ─────────────────────────────────────────────
    ...
    if not metrics:
        ...
        c2.metric("Trades Vencedores", str(metrics['trades_lucro']))
        c3.metric("Trades Perdedores", str(metrics['trades_perda']))
        ...
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Maior Sequência de Ganhos", f"{metrics['max_seq_win']} trades")
        c2.metric("Maior Sequência de Perdas", f"{metrics['max_seq_loss']} trades")
        ...
        c1, c2, c3 = st.columns(3)
        c1.metric("Retorno Médio/Trade", f"{metrics['retorno_medio_pct']:+.2f}%")
